  ...
  fetch(root + 'data/projects.json')
    ...
    .then(function (data) {
      ...
      container.innerHTML = projects.map(function (p) {
        var title = lang === 'en' ? p.title_en : p.title_fa;
        var role = lang === 'en' ? p.role_en : p.role_fa;
        return (
          '<div class="card" style="padding:0;overflow:hidden;">' +
            '<div class="post-thumb" style="height:170px;">' +
              '<img src="' + root + p.image + '" alt="' + title + '" ' +
                'style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;">' +
            '</div>' +
            '<div style="padding:20px 22px;">' +
              '<h3 style="font-size:16.5px;margin:0 0 8px;">' + title + '</h3>' +
              '<p style="color:var(--ink-soft);font-size:14px;margin:0;">' + role + '</p>' +
            '</div>' +
          '</div>'
        );
      }).join('');
